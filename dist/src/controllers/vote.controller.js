...
exports.voteController = {
    ...
    castVote: async (req, res) => {
        try {
            ...
            const [user, election, candidate] = await Promise.all([
                ...
                database_1.default.candidate.findUnique({
                    where: { id: candidateId },
                    select: { id: true, electionId: true, status: true },
                }),
            ]);
            if (!user || user.role !== 'VOTER' || !user.isVerified || !user.isActive || !user.constituencyId) {
                return (0, response_1.sendError)(res, 403, 'You are not eligible to vote');
            }
            if (!election) {
                return (0, response_1.sendError)(res, 404, 'Election not found');
            }
            if (election.status !== 'ACTIVE') {
                return (0, response_1.sendError)(res, 400, 'Voting is not open for this election');
            }
            const now = new Date();
            if (election.startDate > now || election.endDate < now) {
                return (0, response_1.sendError)(res, 400, 'This election is outside the active voting window');
            }
            if (election.constituencyId && election.constituencyId !== user.constituencyId) {
                return (0, response_1.sendError)(res, 403, 'You are not eligible for this constituency election');
            }
            if (!candidate || candidate.electionId !== electionId || candidate.status !== 'APPROVED') {
                return (0, response_1.sendError)(res, 400, 'Selected candidate is not valid for this election');
            }
      ...
      `;
            ...
            if (!voteResult.success) {
                ...
                if (voteResult.error?.includes('already voted')) {
                    await database_1.default.fraudAlert.create({
                        data: {
                            userId,
            ...
            database_1.default.$executeRaw `SELECT refresh_election_results()`.catch(() => { });
    ...
    getReceipt: async (req, res) => {
        try {
            const userId = req.user.userId;
            const receipt = await database_1.default.voteReceipt.findFirst({
                where: {
